...
enum    {
  ...
  CATCFG_T_CYCLE = 15 * 60,        // every 60 minutes
  CATCFG_T_CYCLE_TEST = 30,       // every 30 seconds
  CATCFG_T_CYCLE_INITIAL = 30,    // every 30 seconds initially
  CATCFG_INTERVAL_COUNT_INITIAL = 4,     // repeat for 2 minutes initial
...
static void awaitCb(osjob_t *pSendJob);
...
void setup_uplink(void)
{
        ...
        static_cast<uint32_t>(gCatena.OPERATING_FLAGS::fManufacturingTest)))
  {
    ...
    else
    {
      ...
      gCatena.SafePrintf("Set uplink Lorawan\n");
...
void fillBuffer(TxBuffer_t &b)
{
  ...
  wakeCycleCount++;
  gCatena.SafePrintf("wake count %d \n", (int)wakeCycleCount);

  if (fLight)
    gSi1133.start(true);

  if ((wakeCycleCount%4 + 1 ) == 2)
  {
    b.begin();
  }

  gCatena.SafePrintf("buffer size %d \n" ,b.getn());
  ...
  float vBus = gCatena.ReadVbus(); //declaration put outside if condition
    if ((wakeCycleCount%4 + 1 ) == 2)
  {
...
//    }
  ...
  *pFlag = uint8_t(flag);
  ...
  gCatena.SafePrintf("buffer size after %d \n" ,b.getn());
}
...
void startSendingUplink(void)
{
  if ((wakeCycleCount%4 + 1 ) == 2)
  {
    gCatena.SafePrintf("Buffer cleared -------------------------------- \n");
    TxBuffer_t b;
  }
  
  LedPattern savedLed = gLed.Set(LedPattern::Measuring);

  gCatena.SafePrintf("Filling buffer \n");

  fillBuffer(b);
 
  gCatena.SafePrintf("Buffer Filled \n");
  ...
  if (true)
  {
    ...
    gCatena.SafePrintf("Starting send check \n");
    if (wakeCycleCount % 4) {
      gCatena.SafePrintf("LoRa Pulse NOT Sent  --------- \n");
    ...
    if (!(wakeCycleCount % 4)) {
      gCatena.SafePrintf("LoRa Pulse Send --------\n");
...
)
{
  ...
  {
    gCatena.SafePrintf("System reboot.\n");
    // time to reboot
    NVIC_SystemReset();
  }
